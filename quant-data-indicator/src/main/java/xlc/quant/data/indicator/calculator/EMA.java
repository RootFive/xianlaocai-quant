...
public class EMA extends Indicator {
	...
	private static class EMACalculator extends IndicatorCalculator<EMA> {
		...
		public EMACalculator(int period) {
			super(period, false);
			this.α = DoubleUtils.divide(2, period + 1, 4);
		}
...
//				emaValue = divide((head.getHigh().add(head.getLow()).add(head.getClose())),INT_3, 8);
				emaValue = xlc.quant.data.indicator.util.DoubleUtils.mean(8,head.getHigh(),head.getLow(),head.getClose());
			} else {
				// 当前使用价格
				 ...
				 * 其基本计算公式为：EMA(n)=α×Cn+(1-α)×EMA(n-1) = EMA(n-1) + α * [Cn- EMA(n-1)]
				 */
				Double prevEmaValue = prev.getIndicator().getValue();
				emaValue = DoubleUtils.setScale((headClose-prevEmaValue) *α + prevEmaValue, 2);;
			}
